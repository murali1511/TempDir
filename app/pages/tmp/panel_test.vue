<template>
  <div>
    <!-- Loop categories -->
    <div v-for="(faci, k) in Facilities[0].nearby_facilities" :key="k" class="mb-6 ma-2">
      <v-card>
        <v-card-title class="d-flex align-center bg-orange">
          ...
          <v-btn density="compact" class="ma-2" color="black" icon @click="toggleAll(k)">
            <v-icon>
      ...
      <v-expansion-panels v-model="openPanels[k]" multiple class="rounded-xl mt-2">
        <v-expansion-panel v-for="(s, i) in faci.options || []" :key="i" color="#f2dfa7">
          ...
          <v-expansion-panel-title class="d-flex align-center">
            <div class="d-flex align-center" style="flex: 1">
              <v-icon class="mr-2">{{ faci.icon }}</v-icon>
              <span>{{ s.name }}</span>
            </div>
          ...
          <v-expansion-panel-text class="text-body-2">
            <div class="mb-2 font-weight-bold">{{ s.distance }}</div>

            <div v-if="Array.isArray(s.details)" class="d-flex flex-column gap-2">
              <div v-for="(item, j) in s.details" :key="j" class="d-flex align-center">
                <v-icon class="mr-2" small>mdi-circle-small</v-icon>
                <span>{{ item }}</span>
              </div>
            </div>

            <div v-else>{{ s.details }}</div>
...
onMounted(() => {
  const categories = Facilities[0].nearby_facilities || [];
...
function isAllOpen(k) {
  const options = Facilities[0].nearby_facilities?.[k]?.options || [];
