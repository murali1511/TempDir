...
<div>
    ...
    <div class="calendar">
        ...
        <div class="calendar-grid">
            ...
            <v-btn v-for="(day, index) in daysInMonth" :key="index" class="calendar-cell"
                :class="{ 'other-month': day.isOtherMonth, 'has-program': day.events.length > 0 }"
    ...
    <!-- Event Details Card -->
    <v-card v-if="selectedDay && selectedDay.events.length > 0" class="mt-5">
      <v-card-title class="text-center bg-red text-body-1">Events on {{ formatedDate_selectedDay }} </v-card-title>
      <v-divider></v-divider>
      <v-card-text>
        <v-list>
          <v-list-item
          ...
          >
            <v-list-item-content>
              ...
              <v-list-item-subtitle>Color: {{ event.color }}</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </v-list>
      </v-card-text>
...
const formatedDate_selectedDay = ref('')

const events = ref([
  { name: "Meeting", date: "2024-11-22", color: "blue" },
  { name: "Workshop", date: "2024-11-18", color: "green" },
  { name: "Conference", date: "2024-11-19", color: "red" },
]);
...
const daysInMonth = computed(() => {
  ...
  for (let i = 0; i < totalDays; i++) {
    ...
    // Match events with this date using local date string
    const dateString = date.toLocaleDateString("en-CA"); // YYYY-MM-DD format
    const matchingEvents = events.value.filter((event) => event.date === dateString);
...
const handleDayClick = (day) => {
  ...
  formatedDate_selectedDay.value = new Date(day.dateString).toLocaleDateString("en-GB", {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
